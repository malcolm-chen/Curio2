...
@db_viewer.route('/api/conversations/<conversation_id>/messages/<message_id>/audio', methods=['GET'])
...
def export_all_data():
    ...
    try:
        ...
        if format_type == 'json':
            ...
            # Header
            writer.writerow([
                'Conversation ID', 'Session ID', 'Phenomenon', 'Started At',
                'Finished At', 'Message Count', 'Evaluation Result'
            ])
...
def get_statistics():
    ...
    try:
        ...
        conversations_with_audio = db.query(Message.conversation_id).filter(
            Message.audio_data.isnot(None)
        ).distinct().count()
        total_audio_messages = db.query(Message).filter(
            Message.audio_data.isnot(None)
        ).count()
