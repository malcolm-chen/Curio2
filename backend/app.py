...
if postgres_user and postgres_password and postgres_host and postgres_db:
    ...
    print(f"  User: {postgres_user}, Host: {postgres_host}, Port: {postgres_port}, Database: {postgres_db}")
else:
...
def chat_completion():
    ...
    try:
        ...
        session_identifier = (data.get('session_id') or request_session_id or '').strip()
        ...
        if user_audio_b64:
            try:
                ...
                print(f"Failed to decode user audio for conversation {conversation_id}: {audio_error}")
        ...
        if state != 'scienceqa':
            ...
            qualified_question_num = sum(
                1 for question in conv_scienceqa_history if question in ['explanatory', 'general_causal', 'specific_causal']
            )
